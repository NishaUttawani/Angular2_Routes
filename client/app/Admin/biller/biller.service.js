"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var http_1 = require("@angular/http");
require("rxjs/add/operator/map");
var Biller = (function () {
    function Biller(id, billerName, billerType, officeAddress) {
        if (officeAddress === void 0) { officeAddress = {
            email: '',
            streetAdress: '',
            zipCode: 0,
            city: '',
            country: '',
            phone: 0
        }; }
        this.id = id;
        this.billerName = billerName;
        this.billerType = billerType;
        this.officeAddress = officeAddress;
    }
    return Biller;
}());
exports.Biller = Biller;
var BillerService = (function () {
    function BillerService(http) {
        this.http = http;
    }
    BillerService.prototype.getBillers = function () {
        return this.http.get('/api/billers')
            .map(function (res) { return res.json(); });
    };
    BillerService.prototype.addBiller = function (newBiller) {
        var headers = new http_1.Headers();
        headers.append('Content-Type', 'application/json');
        return this.http.post('/api/billers', JSON.stringify(newBiller), { headers: headers })
            .map(function (res) { return res.json(); });
    };
    BillerService.prototype.getBiller = function (id) {
        var obj = { "id": id };
        var headers = new http_1.Headers();
        headers.append('Content-Type', 'application/json');
        return this.http.post('/api/biller/', JSON.stringify(obj), { headers: headers })
            .map(function (res) { return res.json(); });
    };
    BillerService.prototype.editBiller = function (editedBiller) {
        var headers = new http_1.Headers();
        headers.append('Content-Type', 'application/json');
        return this.http.post('/api/editbiller', JSON.stringify(editedBiller), { headers: headers })
            .map(function (res) { return res.json(); });
    };
    return BillerService;
}());
BillerService = __decorate([
    core_1.Injectable(),
    __metadata("design:paramtypes", [http_1.Http])
], BillerService);
exports.BillerService = BillerService;
//# sourceMappingURL=biller.service.js.map